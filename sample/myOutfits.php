...
<html>
      ...
      <script type='text/javascript'>
        $(document).ready(function(){
          $('.delBtn').click(function(){         
            var postid = $(this).attr('data-oName');
            //alert (postid);
            $.ajax({
              type     : 'post',
              url      : 'deleteOutfit.php',
              data     : {
                  "oName": postid
              },
    ...
    <body>
        ...
        <header class="text-center">
          <h2 class="h5 text-uppercase mb-4">My Outfits</h2>
          </header>
        <div class="row">
            <?php
            for ($x = 0; $x < count($oArray); $x++) {
                echo '<div class="col-lg-4 col-sm-6">
                <div class="row">
                <div class="col-lg-4 col-sm-6">
                  <div class="product text-center">
                    <div class="mb-3 position-relative">
                      <img class="img-fluid w-100" src="'.$oArray['prod'.$x]['top']['img_URL'].'">
                    </div>
                  </div>
                </div>
                  <div class="col-lg-4 col-sm-6">
                  <div class="product text-center">
                    <div class="mb-3 position-relative">
                      <img class="img-fluid w-100" src="'.$oArray['prod'.$x]['bottom']['img_URL'].'">
                    </div>
                  </div>
                  </div>
                </div>
                <h6> <a class="reset-anchor text-center" >'.$oArray['prod'.$x]['outfitName']['name'].'</a></h6>
                <br>
                <a class="btn delBtn" data-oName="'.$oArray['prod'.$x]['outfitName']['name'].'">Delete</a>
                </div>';
            }
            ?>
